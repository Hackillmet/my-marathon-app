...
// Получаем ID пользователя для локального хранилища
...
const STORAGE_KEYS = {
    ...
    CUSTOM_WORKOUTS: `custom_workouts_${userId}`,
    THEME: `theme_${userId}`,
    LANGUAGE: `language_${userId}`
};

// ===== 30 БАЗОВЫХ ТРЕНИРОВОК =====
const BASE_WORKOUTS = {
    1: {
        id: 1,
...
// Добавляем тренировки с 16 по 30
for (let i = 16; i <= 30; i++) {
    const sourceDay = ((i - 1) % 15) + 1;
    BASE_WORKOUTS[i] = {
...
// Хранилище для пользовательских тренировок
let customWorkouts = [];
...
function loadData() {
    ...
    customWorkouts = JSON.parse(localStorage.getItem(STORAGE_KEYS.CUSTOM_WORKOUTS)) || [];
...
function updateUI() {
    ...
    if (!dayStarted) {
        ...
        if (dayCompletedTime && !canStart) {
            ...
            document.getElementById('time-info').textContent = t('nextDayIn', remaining.hours, remaining.minutes);
        } else if (!canStartByTime) {
            btn.disabled = true;
            btn.textContent = t('waitUntil4am');
            document.getElementById('time-info').textContent = t('waitUntil4am');
        } else {
            btn.disabled = false;
            btn.textContent = t('startBtn');
            document.getElementById('time-info').textContent = t('canStart');
        }
...
function renderWorkout() {
    if (!currentWorkout) {
        // Сначала ищем в пользовательских тренировках для этого дня
        const customForDay = customWorkouts.find(w => w.day === currentDay);
        
        if (customForDay) {
            currentWorkout = {
                id: customForDay.id,
                name: customForDay.name,
                difficulty: customForDay.difficulty || 'medium',
                steps: customForDay.steps.map(s => ({...s, completed: false})),
                totalDistance: customForDay.totalDistance
            };
        } else {
            // Если нет пользовательской, берем из базовых
            const template = BASE_WORKOUTS[currentDay] || BASE_WORKOUTS[((currentDay - 1) % 30) + 1];
            currentWorkout = {
                id: currentDay,
                name: template.name,
...
function updateProgress() {
    ...
    // Обновляем информацию о дедлайне
    updateDeadlineInfo();
}
...
// ========== ФУНКЦИЯ ДОБАВЛЕНИЯ СВОЕЙ ТРЕНИРОВКИ ==========
function addCustomWorkout() {
    tg.showPopup({
        title: '➕ Новая тренировка',
        message: 'Введите название тренировки',
        buttons: [
            { id: 'next', type: 'default', text: 'Далее' },
            { type: 'cancel' }
        ]
    }, (buttonId) => {
        if (buttonId === 'next') {
            // Здесь можно добавить форму для создания тренировки
...
function updateAllText() {
    ...
    if (aboutInfo) {
        aboutInfo.innerHTML = `
            ...
            <p>Беговой марафон - 30+ тренировок</p>
        `;
    }
...
window.addCustomWorkout = addCustomWorkout;
